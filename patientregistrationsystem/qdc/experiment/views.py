...
@login_required
@permission_required("experiment.register_equipment")
def mriscanner_list(request, template_name="experiment/mriscanner_list.html"):
    return render(
        request,
        template_name,
        {"equipments": MRIScanner.objects.all().order_by("manufacturer_model_name")},
    )
...
def mriscanner_create(request, template_name="experiment/mriscanner_register.html"):
    ...
    mriscanner_form = MRIScannerForm(
        request.POST or None, initial={"equipment_type": "mri_machine"}
    )
    ...
    if request.method == "POST":
        ...
        if request.POST["action"] == "save":
            ...
            if mriscanner_form.is_valid():
                ...
                mriscanner_added = mriscanner_form.save(commit=False)
                mriscanner_added.equipment_type = "mri_machine"
                mriscanner_added.save()
        ...
        if request.POST["action"] == "save":
            ...
            if additional_data_form.is_valid():
                ...
                for file_to_upload in files_to_upload_list:
                    ...
                    idFormato = FileFormat.objects.filter(
                        name=additional_data_form.instance.file_format
                    )
